...
#RUTAS DE PROFESORES REPORTES
@app.route('/profesores/reportes', methods=['GET', 'POST'])
def reportes_profesores():
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)

    # ========== GUARDAR REPORTE ==========
    if request.method == 'POST':
        profesor_id = request.form['persona_id']  # numero_empleado
        motivo = request.form['motivo']
        hora = request.form['hora']
        fecha = request.form['fecha']

        cursor.execute("""
            INSERT INTO reportes_profesores
                (profesor_id, motivo, hora, fecha)
            VALUES (%s, %s, %s, %s)
        """, (profesor_id, motivo, hora, fecha))
    ...
    cursor.execute("""
        SELECT
            ...
            rp.hora,
            rp.fecha,
            p.nombre_docente AS nombre_persona
        FROM reportes_profesores rp
        INNER JOIN profesores p
            ON rp.profesor_id = p.numero_empleado
    ...
    return render_template(
        'reportes_profesores.html',
        profesores=profesores,
        reportes=reportes
    )

@app.route('/profesores/reportes/imprimir/<int:id>')
def imprimir_reporte_profesor(id):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    ...
    return render_template(
        'imprimir_reporte_profesor.html',
        reporte=reporte
    )

@app.route('/profesores/reportes/eliminar/<int:id>')
def eliminar_reporte_profesor(id):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "DELETE FROM reportes_profesores WHERE id = %s",
        (id,)
    )
    mysql.connection.commit()
    cursor.close()

    return redirect('/profesores/reportes')
...
def recursos_route():
    ...
    # ================= SUBIR RECURSO (SOLO PROFESOR) =================
    if request.method == 'POST':

        # Seguridad: solo profesores pueden subir
        if session.get('tipo_usuario') != 'profesor':
            flash('No tienes permiso para subir recursos.')
        ...
        if execute_update("""
            INSERT INTO recursos
                (nombre, tipo_documento, docente, materia, archivo, fecha_subida)
            VALUES
                (%s, %s, %s, %s, %s, %s)
        """, (nombre, tipo_documento, docente, materia, filename, datetime.now())):
            ...
            if nuevo_recurso:
                mensaje = f"📢 Nuevo recurso disponible: {nombre} ({materia}) - {docente}"

                # Crear notificación SOLO para alumnos
                execute_update("""
                    INSERT INTO notificaciones
                        (mensaje, destinatario, recurso_id, fecha, leido)
                    VALUES
                        (%s, %s, %s, %s, 0)
                """, (mensaje, 'alumnos', nuevo_recurso['id'], datetime.now()))

                flash('Recurso subido correctamente y notificado a los alumnos.')
            else:
                flash('Recurso subido, pero no se pudo notificar.')
    ...
    # ================= NOTIFICACIONES (SOLO ALUMNOS) =================
    notificaciones = []
    if session.get('tipo_usuario') == 'alumno':
        notificaciones = fetchall("""
            SELECT id, mensaje, fecha
            FROM notificaciones
            WHERE destinatario = 'alumnos'
              AND leido = 0
            ORDER BY fecha DESC
            LIMIT 1
        """)
...
def consultas():
    ...
    alumnos = fetchall("""
        SELECT
            ...
            turno
        FROM alumnos
        ORDER BY grupo, apellido_paterno
    """)
    ...
    reportes_alumnos = fetchall("""
        ...
        FROM reportes r
        INNER JOIN alumnos a ON r.persona_id = a.id
        GROUP BY a.id
    """)

    # ================== 4️⃣ SUSPENSIONES ==================
    suspensiones = fetchall("""
        SELECT 
            CONCAT(a.nombre, ' ', a.apellido_paterno, ' ', a.apellido_materno) AS alumno,
            COUNT(r.id) AS reportes,
            IF(COUNT(r.id) >= 3, 'Suspendido', 'Advertencia') AS estado,
    ...
    # ================== 5️⃣ REPORTES A PROFESORES ==================
    reportes_profesores = fetchall("""
        SELECT 
            p.nombre_docente AS profesor,
            COUNT(rp.id) AS total_reportes,
            MAX(rp.fecha) AS ultimo_reporte
        FROM reportes_profesores rp
        INNER JOIN profesores p ON rp.profesor_id = p.numero_empleado
        GROUP BY p.numero_empleado
    """)

    return render_template(
        'consultas.html',
        alumnos=alumnos,   # 👈 ESTE ES EL IMPORTANTE
        listado=listado,
        reportes_alumnos=reportes_alumnos,
        suspensiones=suspensiones,
        reportes_profesores=reportes_profesores
    )
#Apartado de chat :)
...
def enviar():
    ...
    mensaje = {"tipo": tipo, "contenido": contenido, "hora": hora}
...
# 🔔 CONTADOR DE NOTIFICACIONES (SOLO ALUMNOS)
def contar_notificaciones():
    if session.get('tipo_usuario') != 'alumno' :
        return 0
...
def inject_contador_notificaciones():
    return {
        'contar_notificaciones': contar_notificaciones
    }
